...
const CompanyPartnershipManager = () => {
    ...
    const tabs = [
        { id: 'overview', label: 'Overview', icon: <FaBriefcase />, color: 'emerald' },
        { id: 'capital', label: 'Capital Injection', icon: <FaMoneyBillTrendUp />, color: 'blue' },
        { id: 'approval-queue', label: 'Approval Queue', icon: <FaCircleCheck />, color: 'emerald', adminOnly: true },
        { id: 'products', label: 'Asset Financing', icon: <FaTv />, color: 'amber' },
        { id: 'offset', label: 'Clear Debt (Offset)', icon: <FaUnlockKeyhole />, color: 'rose', adminOnly: true },
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 pb-20 space-y-8 animate-fade-in">
            {/* Header */}
            <div className="bg-gradient-to-r from-emerald-900 via-green-800 to-emerald-900 p-8 md:p-12 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group border border-white/10">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="flex items-center gap-6">
                        <motion.div
                            ...
                            className="p-5 bg-gradient-to-br from-yellow-400 to-amber-600 rounded-3xl shadow-2xl shadow-yellow-600/30"
                        >
                        ...
                        <div>
                            <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-none mb-2">
                                Partnership Hub
                                {matrixStatus && (
                                    <span className={`ml-4 px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest border-2 shadow-xl animate-pulse ${matrixStatus.currentTier.color_code === 'gold' ? 'bg-yellow-400 text-yellow-950 border-yellow-200' :
                                        matrixStatus.currentTier.color_code === 'platinum' ? 'bg-indigo-600 text-white border-indigo-400' :
                                            matrixStatus.currentTier.color_code === 'indigo' ? 'bg-indigo-600 text-white border-indigo-400' :
                                                matrixStatus.currentTier.color_code === 'blue' ? 'bg-blue-600 text-white border-blue-400' :
                                                    'bg-emerald-600 text-white border-emerald-400'
                                        }`}>
                                        {matrixStatus.currentTier.tier_name}
                                    </span>
                                )}
                            </h1>
                            <p className="text-green-100 font-medium opacity-80 text-lg">
                                Institutional Capital & Risk Command Center
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {/* Navigation Tabs */}
            <div className="flex space-x-2 bg-white/40 backdrop-blur-xl p-2 rounded-[2.5rem] border border-white/20 shadow-2xl shadow-emerald-900/5 overflow-x-auto no-scrollbar scroll-smooth">
                {tabs.map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id)}
                        className={`flex items-center gap-3 px-8 py-4 rounded-[2rem] font-black transition-all duration-500 whitespace-nowrap relative group ${activeTab === tab.id
                            ? 'text-white'
                            : 'text-gray-500 hover:text-emerald-600 hover:bg-white/60'
                            }`}
                    >
                        <span className={`relative z-10 transition-transform duration-500 ${activeTab === tab.id ? 'scale-125' : 'group-hover:scale-110'}`}>{tab.icon}</span>
                        <span className="text-sm uppercase tracking-widest relative z-10">{tab.label}</span>
                        {activeTab === tab.id && (
                            <motion.div
                                layoutId="partnership-active-tab"
                                className="absolute inset-0 bg-gradient-to-r from-emerald-600 to-green-500 rounded-[2rem] shadow-lg shadow-emerald-600/30"
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    {/* CONTENT AREA */}
                    <div className={`bg-white/80 backdrop-blur-md rounded-[3rem] shadow-2xl shadow-emerald-900/10 border border-white/40 min-h-[500px] transition-all duration-700 relative overflow-hidden ${loading ? 'opacity-60 saturate-50' : 'opacity-100'}`}>
                        <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-50 rounded-full blur-3xl opacity-50 -z-10 -translate-y-20 translate-x-20"></div>
                        <div className="absolute bottom-0 left-0 w-96 h-96 bg-blue-50 rounded-full blur-3xl opacity-50 -z-10 translate-y-20 -translate-x-20"></div>
                            ...
                            <div className="p-6 md:p-10 space-y-10">
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl shadow-lg transition-all font-black text-xs uppercase tracking-widest relative group overflow-hidden"
                                    >
                                        ...
                                        <div className="relative z-10">
                                            <div className="flex items-center gap-4 mb-8">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-400 font-bold uppercase tracking-widest">Strict {matrixStatus?.funding?.multiplier || 5}x Institutional Injection</p>
                                                </div>
                                            </div>
                                            ...
                                            <form onSubmit={handleSubmitTopUp(onTopUpRequestSubmit)} className="space-y-6">
                                                        ...
                                                        {matrixStatus?.funding?.currentCommitment > 0 && (
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    setAutoCalculatedTopUp(matrixStatus.funding.currentCommitment * (matrixStatus.funding.multiplier || 5));
                                                                }}
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            {...registerTopUp('commitmentAmount', {
                                                                required: true,
                                                                onChange: (e) => setAutoCalculatedTopUp(parseFloat(e.target.value) * (matrixStatus?.funding?.multiplier || 5))
                                                            })}
                                                            type="number"
                                                ...
                                                <div className="bg-blue-50/50 border-2 border-blue-100 p-6 rounded-[2rem] flex items-center justify-between">
                                                    <div>
                                                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest leading-none mb-2">Calculated Top-Up ({matrixStatus?.funding?.multiplier || 5}x)</p>
